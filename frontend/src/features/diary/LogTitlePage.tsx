...
export default function LogTitlePage() {
  ...
  const [selectedTitle, setSelectedTitle] = useState<SearchResult | Title | null>(
  ...
  const [watchedAt, setWatchedAt] = useState<string>(
    location.state?.watchedAt || new Date().toISOString().slice(0, 10)
  );
  const [rating, setRating] = useState<number | null>(() => {
    ...
    const existingWatch = watchLogs.find(
      (l) =>
        ("id" in initialTitle && l.title.id === initialTitle.id) ||
        (initialTitle.tmdb_id && l.title.tmdb_id === initialTitle.tmdb_id),
    );
    return existingWatch ? (personalRatings[existingWatch.title.id] ?? null) : null;
  });
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        ...
        description={selectedTitle ? "Record your watch details below." : "Search for a title and fill in your watch details."}
      />
      ...
      >
        {/* Title Selection Card (Integrated Search or Selected Preview) */}
        <div className={styles.fieldCard}>
          {selectedTitle ? (
            <span className={styles.fieldLabel}>
              Title <span className={styles.labelHint}>· required</span>
            </span>
          ) : (
            <label htmlFor="title-search" className={styles.fieldLabel}>
              Title <span className={styles.labelHint}>· required</span>
            </label>
          )}

          {selectedTitle ? (
            <div className={styles.titlePreview} style={{ margin: 0 }}>
            ...
            <div className={styles.searchFormWrapper} ref={searchContainerRef}>
              <SearchInput
                id="title-search"
                value={query}
                onChange={handleQueryChange}
                placeholder="Type a movie or series title to select…"
              />
        ...
        <button
          ...
          title={!selectedTitle ? "Please select a title first" : undefined}
        >
          {isSubmitting ? (
            <>
